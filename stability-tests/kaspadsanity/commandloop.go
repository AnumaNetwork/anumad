...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("anumadsanity-anumad-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("anumad", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "ANUMAD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "ANUMAD-STDERR")
