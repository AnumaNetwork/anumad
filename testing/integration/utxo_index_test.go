...
func TestUTXOIndex(t *testing.T) {
	// Setup a single anumad instance
	...
	err := anumad.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := anumad.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedSompi := uint64(blockAmountToMine * constants.SompiPerAnuma * 500)
	getBlockCountResponse, err := anumad.rpcClient.GetBlockCount()
	if err != nil {
		t.Fatalf("Error Retriving BlockCount: %s", err)
	}
	rewardsMinedViaBlockCountSompi := uint64(
		(getBlockCountResponse.BlockCount - 2) * constants.SompiPerAnuma * 500, // -2 because of genesis and virtual.
	)
	...
	utxosByAddressesResponse, err := anumad.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) (*appmessage.RPCTransaction, string) {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixAnumaSim)
	if err != nil {
