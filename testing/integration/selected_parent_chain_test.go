...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of anumad instances
	anumad1, anumad2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := anumad1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In anumad2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, anumad2)
	}

	// Connect the two anumads. This should trigger sync
	// between the two nodes
	connect(t, anumad1, anumad2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := anumad1.rpcClient.GetVirtualSelectedParentChainFromBlock(
